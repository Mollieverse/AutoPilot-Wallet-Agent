...
import {
  ...
  ArrowRight, Shield, Activity, ChevronRight,
} from 'lucide-react';
...
function LandingPage() {
  const steps = [
    {
      ...
      desc:  'Link your Phantom or Solflare wallet. HELM never holds your funds — you stay in full control at all times.',
    },
    {
      icon:  '🤖',
    ...
    {
      icon:  '📡',
      title: 'HELM Monitors 24/7',
      desc:  'Your agent watches live market prices continuously. No need to stare at charts — HELM does it for you.',
    },
    {
      icon:  '⚡',
      title: 'Auto-Executes On-Chain',
      desc:  'When your condition is met, HELM builds the transaction. You approve it in your wallet — one tap.',
    },
  ];
  ...
  const features = [
    ...
    },
    {
      ...
      label: 'Multi-agent',
      desc:  'Run multiple strategies at once.',
    },
    {
      icon:  <Activity className="w-4 h-4" />,
      label: 'Live price data',
      desc:  'Real-time via Jupiter Price API.',
    },
    {
      icon:  <Zap      className="w-4 h-4" />,
      label: 'Instant execution',
      desc:  'On-chain in seconds via Jupiter.',
    },
  ];
      ...
      <header className="sticky top-0 z-30 bg-bg/90 backdrop-blur-xl border-b border-border">
        <div className="flex items-center justify-between px-5 h-14 max-w-4xl mx-auto w-full">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="font-bold text-sm leading-none">HELM</p>
              <p className="text-[10px] text-muted leading-none">Intent Execution</p>
            </div>
          </div>
          <WalletButton />
        </div>
      ...
      <div className="flex-1 px-5 py-8 max-w-2xl mx-auto w-full space-y-10">
        ...
        <div className="text-center space-y-4 pt-4">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full
                          bg-accent/10 border border-accent/20 text-xs font-mono
          ...
          <p className="text-muted text-base leading-relaxed max-w-md mx-auto">
            HELM monitors Solana token prices 24/7 and automatically
            executes trades when your conditions are met —
            without you lifting a finger. Your keys never leave Phantom.
          </p>
          ...
          <div className="space-y-3">
            {steps.map((step, i) => (
              ...
              >
                ...
                {i < steps.length - 1 && (
                  <ChevronRight className="w-4 h-4 text-muted/30 flex-shrink-0
          ...
          <div className="flex flex-wrap gap-2 text-xs">
            {[
              ...
              <div key={i} className="flex items-center gap-1.5">
                ...
                {i < 4 && (
                  <ArrowRight className="w-3 h-3 text-muted/40 flex-shrink-0" />
                )}
              </div>
            ))}
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {features.map(f => (
            ...
            >
              <div className="w-8 h-8 rounded-lg bg-primary/15 text-primary
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap justify-center gap-2">
            {[
              'SOL', 'JTO', 'RAY', 'BONK', 'WIF',
              'JUP', 'PYTH', 'RENDER', 'HNT', 'ORCA',
              '+10 more',
            ].map(t => (
              <span
                key={t}
        ...
        <div className="text-center pb-8 space-y-3">
          ...
          <p className="text-xs text-muted">
            Running on{' '}
            <span className="text-accent font-semibold">Mainnet</span>{' '}
            — real Jupiter swaps executed on-chain.
          </p>
        </div>
      </div>
    </div>
        ...
        {connected && (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg
                       ...
                       hover:bg-primary/25 transition-all border border-primary/20
                       active:scale-95"
          >
