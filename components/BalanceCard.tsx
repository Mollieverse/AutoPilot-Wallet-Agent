...
export const BalanceCard = memo(function BalanceCard({
  balance, loading, refresh, price,
}: Props) {
  const { connected, publicKey } = useWallet();
  const positive = (price?.change24h ?? 0) >= 0;
  const usdValue = balance != null && price
    ? (balance * price.current).toFixed(2)
    : null;
    ...
    <div className="relative rounded-2xl overflow-hidden border border-border glow-purple">
      ...
      <div className="relative p-5 sm:p-6">
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          <button
            onClick={refresh}
            className={`p-1.5 rounded-lg hover:bg-card-2 text-muted hover:text-text
        ...
        {!connected ? (
          <div>
            ...
            <p className="text-sm text-muted mt-1">Connect wallet to view balance</p>
          </div>
        ) : loading ? (
        ...
        <div className="mt-5 pt-4 border-t border-border flex items-center justify-between">
          {!price ? (
            /* Skeleton while real price loads — no fake $148.50 */
            <div className="flex items-center gap-3 flex-1">
              <div>
                <p className="text-xs text-muted mb-1">SOL / USD</p>
                <div className="h-6 w-24 rounded-lg bg-card-2 shimmer" />
              </div>
              <div className="h-6 w-16 rounded-full bg-card-2 shimmer" />
            </div>
          ) : (
            <>
