'use client';
import {
  Home, Bot, BarChart2, Settings,
  Zap, Activity, PieChart, Layers,
} from 'lucide-react';
...
export function Sidebar({ activeTab, onTabChange, agentCount }: Props) {
  return (
    <aside className="hidden lg:flex flex-col w-60 xl:w-64 h-screen sticky
                      top-0 border-r border-border bg-card/60 backdrop-blur-xl
                      flex-shrink-0">
      {/* Logo */}
      <div className="p-5 border-b border-border">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-primary
          ...
          <div>
            ...
            <p className="text-[9px] text-muted leading-tight uppercase
                          tracking-widest">
              Intent Layer
            </p>
          </div>
      ...
      <div className="px-4 py-3 mx-3 mt-3 rounded-xl bg-card-2 border border-border">
        <div className="flex items-center gap-2">
          ...
          <span className="ml-auto text-xs font-mono font-bold text-accent">
            LIVE
          </span>
        </div>
        <div className="mt-1.5 flex items-center gap-2">
          <span className="text-xs text-muted">Active intents</span>
