...
export function StatsBar({ agentCount, execCount, successRate }: Props) {
  const stats = [
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
      {stats.map(({ label, value, suffix, Icon, color }) => (
        <div
          key={label}
          className="rounded-xl bg-card border border-border p-3.5 flex items-center gap-3
                     hover:border-primary/20 transition-colors animate-fade-in"
        >
          <div className={`w-8 h-8 rounded-lg bg-card-2 flex items-center justify-center flex-shrink-0 ${color}`}>
            <Icon className="w-4 h-4" />
          </div>
          <div className="min-w-0">
            <p className={`font-bold text-base font-mono leading-none ${color}`}>
              {value}{suffix}
            </p>
